...
from src.client.client_control_functions import encrypt_field, derive_aes_key, decrypt_field

import gettext
_ = gettext.gettext

###########################################################################
## Class CredentialFormEdit
###########################################################################

class CredentialFormEdit ( wx.Frame ):
    ...
    def __init__( self, parent, cred ):
        ...
        self.m_textCtrl3 = wx.TextCtrl( self, wx.ID_ANY, self.cred.get_title(), wx.DefaultPosition, wx.DefaultSize, 0 )
        ...
        self.m_textCtrl4 = wx.TextCtrl( self, wx.ID_ANY, self.cred.get_username(), wx.DefaultPosition, wx.DefaultSize, 0 )
        ...
        self.m_textCtrl5 = wx.TextCtrl( self,
                                        wx.ID_ANY,
                                        decrypt_field(self.cred.get_password().hex(), self.cred.get_nonce().hex(),
                                                      self.parent.key).decode('utf-8'),
    ...
    def save_credential(self, event):
        ...
        self.cred.set_title(title)
        self.cred.set_username(user)
        self.cred.set_password(password_encrypted)
        self.cred.set_last_modified(now)
        self.cred.set_nonce(nonce)

        # Update the grid row
        row_idx = self.parent.cred_list.index(self.cred)
        self.parent.m_dataViewListCtrl2.SetTextValue(title, row_idx, 0)
        self.parent.m_dataViewListCtrl2.SetTextValue(user, row_idx, 1)
        self.parent.m_dataViewListCtrl2.SetTextValue(str(now.date()), row_idx, 2)
